module Nis::Serializer
  def serialize_transaction(entity)
...
# };
  ...
  def serialize_safe_string(str)
    str = '' if str.nil?
    [str.size, 0, 0, 0] + str.unpack('C*')
  end
# /* safe string - each char is 8 bit */
...
# };
  ...
  def serialize_ua_string(str)
    str = '' if str.nil?
    [str.size, 0, 0, 0] + str.chars.map(&:to_i)
  end
...
# };
#
  def serialize_mosaic_id(mosaic_id)
...
# };
#
  def serialize_mosaic_and_quantity(mosaic)
...
# };
#
  def serialize_properties(entity)
...
# };
#
  def serialize_mosaic_definition(entity)
